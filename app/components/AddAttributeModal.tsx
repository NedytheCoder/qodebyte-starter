...
interface AddAttributeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddAttribute: (attribute: AttributeFormData) => void;
}

export function AddAttributeModal({ isOpen, onClose, onAddAttribute }: AddAttributeModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAddAttribute({ id: `${Date.now()}`, name: name.trim() });
    setName("");
    setError("");
    onClose();
  };
        ...
        <div className="px-6 py-4 rounded-t-xl bg-gray-700 text-white flex items-start justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs opacity-90">Create a new product attribute.</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="pt-2 grid grid-cols-1 md:grid-cols-[1fr_1.3fr] gap-3">
            <RegularButton
              ...
              type="button"
            />
            <RegularButton label="Save Attribute" type="submit" className="w-full !py-3" />
          </div>
        </form>
      </div>
    </div>
